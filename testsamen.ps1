...
# Script 2: Process user records from the array
...
while ($true) {
    if ($global:userRecords.Count -gt 0) {
        $eersteGebruiker = $global:userRecords[0]
        $voornaam = $eersteGebruiker.Voornaam
        $achternaam = $eersteGebruiker.Achternaam
        $afdeling = $eersteGebruiker.Afdeling
        $email = $eersteGebruiker.Email
        $samAccountName = $eersteGebruiker.SamAccountName
        $wachtwoord = $eersteGebruiker.Wachtwoord
        ...
        try {
            ...
            # Verwijder de eerste gebruiker uit de array na succesvolle domeinjoin
            $global:userRecords = $global:userRecords | Where-Object { $_ -ne $eersteGebruiker }

            Write-Host "Eerste gebruiker succesvol verwerkt en verwijderd uit de array."
        } catch {
            Write-Host "Er is een fout opgetreden bij het aanmaken van de VM: $_"
        }
    } else {
        Write-Host "Geen gebruikers gevonden in de array. Wacht op nieuwe invoer."
        Start-Sleep -Seconds 60
    }
}
